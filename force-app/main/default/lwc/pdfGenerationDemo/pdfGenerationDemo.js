...
import mokshasri_logo from '@salesforce/resourceUrl/mokshasri_logo'

export default class PdfGenerationDemo extends LightningElement {
    @api recordId
    imageUrl = mokshasri_logo
    invoiceData={
        invoiceNo:'123',
    ...
    pdfHandler(){
        ...
        generatePDF({ recordId:this.recordId, htmlData:content.outerHTML}).then(result=>{
            ...
            window.open(`https://lwccom113-dev-ed.develop.file.force.com/servlet/servlet.FileDownload?file=${result.Id}`)
        }).catch(error=>{
            console.error(error)
        })
    }
